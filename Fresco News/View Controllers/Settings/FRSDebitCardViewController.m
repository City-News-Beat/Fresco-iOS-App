                                                    ...
                                                    completion:^(id responseObject, NSError *error) {
                                                      if (error) {
                                                          ...
                                                          if (response && response.statusCode == 500) {
                                                              ...
                                                              self.alertView = [[FRSAlertView alloc] initWithTitle:@"ERROR" message:errorMessage actionTitle:@"OK" cancelTitle:@"" cancelTitleColor:nil delegate:self];
                                                          } else {
                                                              self.alertView = [[FRSAlertView alloc] initWithTitle:@"ERROR" message:error.localizedDescription actionTitle:@"OK" cancelTitle:@"" cancelTitleColor:nil delegate:self];
                                                          }
                                                          [self.alertView show];
                                                          ...
                                                          if (debitCard) {
                                                              if ([[responseObject valueForKey:@"valid"] boolValue]) {
                                                                  ...
                                                                  self.alertView = [[FRSAlertView alloc] initWithTitle:@"ERROR" message:@"The card you entered was invalid. Please try again." actionTitle:@"OK" cancelTitle:@"" cancelTitleColor:nil delegate:self];
                                                                  [self.alertView show];
                                                              }
                                                          } else {
                                                              NSString *creditCard = [NSString stringWithFormat:@"%@ %@", brand, last4];
...
- (void)didSaveDebitCardButtonPressed:(NSString *)cardNumber expDate:(NSString *)expDate cvv:(NSString *)cvv {
    ...
    if (components.count == 2) {
        ...
        self.alertView = [[FRSAlertView alloc] initWithTitle:@"ERROR" message:@"Please make sure your expiration date you entered is correct and try again." actionTitle:@"OK" cancelTitle:@"" cancelTitleColor:nil delegate:self];
        [self.alertView show];
        [self stopSpinner:self.loadingView onButton:self.addDebitCardView.saveButton];
        return;
    }
    ...
    if (!params) {
        self.alertView = [[FRSAlertView alloc] initWithTitle:@"ERROR" message:@"Please check your card information and try again." actionTitle:@"OK" cancelTitle:@"" cancelTitleColor:nil delegate:self];
        [self.alertView show];
        [self stopSpinner:self.loadingView onButton:self.addDebitCardView.saveButton];
        return;
    }
...
- (void)didSaveBankButtonPressed:(NSString *)accountNumber routingNumber:(NSString *)routingNumber {
    ...
    if (!bankParams) {
        self.alertView = [[FRSAlertView alloc] initWithTitle:@"ERROR" message:@"Please make sure your bank information is correct and try again." actionTitle:@"OK" cancelTitle:@"" cancelTitleColor:nil delegate:self];
        [self.alertView show];
        [self stopSpinner:self.loadingView onButton:self.addDebitCardView.saveButton];
        return;
    }
