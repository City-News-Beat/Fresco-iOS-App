...
- (instancetype)initUploadFailAlertViewWithError:(NSError *)error {
    ...
    if (self) {
        ...
        self.titleLabel.text = [uploadError uppercaseString];
        ...
        NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:error.localizedDescription != nil ? error.localizedDescription : @"Unable to upload gallery. Please try again later."];
        ...
        [self.cancelButton addTarget:self action:@selector(getHelp) forControlEvents:UIControlEventTouchUpInside];
        [self.cancelButton setTitleColor:[UIColor frescoBlueColor] forState:UIControlStateNormal];
        [self.cancelButton setTitle:@"GET HELP" forState:UIControlStateNormal];
